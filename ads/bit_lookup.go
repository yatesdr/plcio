...
// No parent value read or read/modify/write is involved.
func (c *Client) validateBitLookup(name string, record *symbolRecord, budget *parseBudget) error {
	info := record.info
	mismatch := func() error {
		return fmt.Errorf("symbol lookup identity mismatch: got %q, want %q", info.Name, name)
	}
	snapshot := c.snapshot
	if snapshot == nil || info.IndexGroup != 0x4041 || info.TypeCode != TypeBool || info.Size != 1 || info.TypeName != "BIT" || info.Flags&SymFlagTypeGUID == 0 || info.Flags&(SymFlagInterfacePointer|SymFlagReferenceTo) != 0 || record.unsupported != "" || !strings.HasPrefix(name, info.Name+".") {
		return mismatch()
	}
	...
	path := strings.TrimPrefix(name, info.Name+".")
	depth := uint32(1)
	for path != "" {
